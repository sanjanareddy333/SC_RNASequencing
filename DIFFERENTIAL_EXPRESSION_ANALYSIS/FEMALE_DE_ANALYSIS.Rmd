...
# Output directory
output_dir <- "/Users/preddy/Desktop/Lab_Streeter_new/DE_ANALYSIS/FEMALE_ITS2_DE_RESULTS"
...
############################################################
## APPLY GENE FILTERING (CORRECT OBJECT)
############################################################

cat("Applying gene filtering...\n")

all_genes <- rownames(female_filtered)

genes_to_exclude <- get_genes_to_exclude(all_genes)

cat("Genes before filtering:", length(all_genes), "\n")
cat("Genes to exclude:", length(genes_to_exclude), "\n")

female_filtered <- subset(
  female_filtered,
  features = setdiff(all_genes, genes_to_exclude)
)

cat("Genes after filtering:", nrow(female_filtered), "\n\n")
...
write.csv(data.frame(gene = A_only),
          file.path(output_dir, "Healthy_WT_only_37genes.csv"),
          row.names = FALSE)
...
write.csv(data.frame(gene = C_only),
          file.path(output_dir, "Healthy_RRAC_vs_REV_only_32genes.csv"),
          row.names = FALSE)

write.csv(data.frame(gene = A_C),
          file.path(output_dir, "Healthy_WT_and_RRAC_vs_WT_8genes.csv"),
          row.names = FALSE)
...
cat("  - Healthy_WT_only_37genes.csv\n")
cat("  - Healthy_RRAC_vs_WT_only_41genes.csv\n")
cat("  - Healthy_RRAC_vs_REV_only_32genes.csv\n")
cat("  - Healthy_WT_and_RRAC_vs_WT_8genes.csv\n")
...
write.csv(
  ...
  file.path(output_dir, "Disease_HighCholesterol_only_64genes.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(output_dir, "Disease_Inflammation_only_31genes.csv"),
  row.names = FALSE
)

cat("✓ Saved disease gene lists:\n")
cat("  - Disease_HighCholesterol_only_64genes.csv\n")
cat("  - Disease_Overlap_Both_13genes.csv\n")
cat("  - Disease_Inflammation_only_31genes.csv\n")
