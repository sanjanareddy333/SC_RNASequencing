---
title: "MALE_PCA_ANALYSIS_FILTERED_GENES"
...
root_dir <- "/Users/preddy/Desktop/Lab_Streeter_new/NEW_PCA_ANALYSIS_JAN23/MALE_PCA_FILTERED_GENES_RESULTS"

male_its2_path <- "~/Desktop/Lab_Streeter_new/DATA/MALE/ITS2.csv"

FILE_PREFIX<- "MALE_FILTERED_GENES_"
...
cat("\n=== FILTERING GENES ===\n")
genes_before <- nrow(male)
cat("Total genes before filtering:", genes_before, "\n")

all_genes <- rownames(male)
...
male <- subset(male, features = genes_to_keep)

genes_after <- nrow(male)
...
for (pc in c("PC1","PC2","PC3")) {
 ...
 filename = file.path(dir_map$PCA_Heatmaps, pc, paste0(FILE_PREFIX,"Heatmap_", pc, "_Default.png")),    width = 9, height = 10
  )
...
saveRDS(male, file = file.path(root_dir, "Male_Seurat_filtered.rds"))
cat("✓ Saved: Male_Seurat_filtered.rds\n")
