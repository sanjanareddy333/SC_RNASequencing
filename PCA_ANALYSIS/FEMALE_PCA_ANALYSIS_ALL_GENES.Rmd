---
title: "FEMALE_PCA_ANALYSIS_ALL_GENES"
...
root_dir <- "/Users/preddy/Desktop/Lab_Streeter_new/NEW_PCA_ANALYSIS_JAN23/FEMALE_PCA_WITH_ALL_GENES_RESULTS"

FILE_PREFIX<- "FEMALE_ALL_GENES_"
...
female_csv <- file.path(root_dir, "Female_Metadata_all_genes.csv")
...
############################################################
# Combine WT groups for ellipse drawing
pca_df_plot <- pca_df %>%
  mutate(treatment_ellipse = ifelse(treatment %in% c("WT1", "WT2"), "WT", treatment))

treat_colors <- c(
  WT   = "#1f77b4",  # Combined WT color
  TG1  = "#2ca02c",  # green
  TG2  = "#ff7f0e"   # orange
)
...
p_pc1_pc2 <- ggplot(
  pca_df_plot,
  aes(PC1, PC2, color = treatment_ellipse, group = treatment_ellipse)
) +
  geom_point(size = 3, alpha = 0.9) +
  geom_text_repel(aes(label = slice_id), size = 4, color = "black") +
...
p_pc2_pc3 <- ggplot(
  pca_df_plot,
  aes(PC2, PC3, color = treatment_ellipse, group = treatment_ellipse)
) +
  geom_point(size = 3, alpha = 0.9) +
  geom_text_repel(aes(label = slice_id), size = 4, color = "black") +
  stat_ellipse(type = "t", level = 0.95, linewidth = 0.8) +
...
annotation_col <- data.frame(treatment = pca_df$treatment_ellipse)
...
saveRDS(female, file = file.path(root_dir, "Female_Seurat_all_genes.rds"))
cat("✓ Saved: Female_Seurat_all_genes.rds\n")
